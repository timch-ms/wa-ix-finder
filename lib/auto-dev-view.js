...
const controls = {
  ...
  bmwDealer: document.querySelector("#bmw-dealer"),
...
function matches(listing) {
  return (!controls.year.value || listing.year === Number(controls.year.value))
    ...
    && (!controls.bmwDealer.checked || listing.officialBmwDealer)
...
function renderCard(listing) {
  ...
  if (listing.url) {
    ...
    link.setAttribute(
      "aria-label",
      `Get more information about the ${listing.year} BMW iX at ${listing.dealer}`
    );
  } else {
    link.removeAttribute("href");
    link.textContent = "Dealer link unavailable";
...
function activeFilterEntries() {
  ...
  return [
    ...
    ["bmwDealer", controls.bmwDealer.checked && "BMW dealers only"],
...
notice.textContent = data.staticSnapshot
  ? `Published Auto.dev snapshot from ${data.lastRefreshDate}. This page makes no inventory API calls.`
  : data.refreshError
...
const stats = data.staticSnapshot
  ? [
      ["Auto.dev results", data.apiTotal.toLocaleString()],
...
document.querySelector("#updated-at").textContent = data.updatedAt
  ? `Auto.dev cache updated ${new Intl.DateTimeFormat("en-US", {
      dateStyle: "medium",
      timeStyle: "short"
    }).format(new Date(data.updatedAt))}.`
  : "Auto.dev cache has not been populated.";
