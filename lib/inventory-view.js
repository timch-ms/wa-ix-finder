...
const DEALER_DOMAINS = [
  ...
  "seattlefinestmotors.com",
  "swickardtoyota.com",
  "volvocarsbellevue.com"
];
const MARKETPLACE_DOMAINS = ["autolist.com"];
const CARFAX_DOMAINS = ["carfax.com"];
...
function listingDestination(value) {
  const dealerUrl = trustedUrl(value, DEALER_DOMAINS);
  if (dealerUrl) return { type: "dealer", url: dealerUrl };
  const marketplaceUrl = trustedUrl(value, MARKETPLACE_DOMAINS);
...
function renderCard(listing) {
  ...
  if (destination.type === "dealer" || destination.type === "marketplace") {
    ...
    dealerLink.setAttribute(
      "aria-label",
      `${dealerLink.textContent} for the ${listing.year} BMW iX at ${listing.dealer}`
    );
  } else {
  ...
  const carfaxUrl = trustedUrl(listing.carfaxUrl, CARFAX_DOMAINS);
  if (carfaxUrl) {
    carfaxLink.href = carfaxUrl;
    carfaxLink.setAttribute(
      "aria-label",
      `View the CARFAX report for the ${listing.year} BMW iX at ${listing.dealer}`
    );
  } else {
